...
ql = QueryLoader("exchange_info.json")
...
def yf_stock_fetch(start_date,end_date,**kwargs):
    ...
    for t in tickers: 
        ...
        df = df.reset_index()                      # DatetimeIndex → 일반 컬럼으로
        df['Ticker'] = t                           # 티커 컬럼 추가
        df_lst.append(df)
...
with DAG(
    dag_id="dag_exchange_rate",
    start_date=datetime(2025,12,1),
    schedule=None,  # 매일 09시
    catchup=True,
    tags = ['Finance','ETL','Exchange'],
    ...
    def prepare_stock_values(**kwargs):
        ...
        values_str = ", ".join([
            f"('{row['Date']}', '{row['Ticker']}', {row['Open']}, {row['High']}, {row['Low']}, {row['Close']}, {row['Volume']},{row['Dividends']},{row['Stock Splits']})"
